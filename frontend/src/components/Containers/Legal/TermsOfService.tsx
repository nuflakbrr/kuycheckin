import { FC } from 'react';
import Head from 'next/head';
import Link from 'next/link';

const ContainerTermsOfService: FC = () => {
  return (
    <>
      <Head>
        <title>Terms Of Service - Wikusama Hotel</title>
      </Head>

      <main className="pt-20">
        <section className="pb-4 lg:py-20">
          <div className="max-w-7xl mx-auto">
            <div className="container">
              <div className="flex flex-wrap items-center justify-start">
                <div className="w-full px-4">
                  <h1 className="text-2xl lg:text-3xl font-bold mb-3 lg:mb-5">
                    Terms and Conditions of Use
                  </h1>

                  <h2 className="text-xl lg:text-2xl font-semibold mb-3 lg:mb-5">
                    1. Terms
                  </h2>

                  <div className="mb-3 lg:mb-5 max-w-3xl">
                    <p className="text-gray-500 xl:text-lg leading-relaxed text-justify mb-5">
                      By accessing this Website, accessible from{' '}
                      <Link href="/" legacyBehavior>
                        <a className="text-blue-500 hover:text-blue-600">
                          https://wikusamahotel.com
                        </a>
                      </Link>
                      , you are agreeing to be bound by these Website Terms and
                      Conditions of Use and agree that you are responsible for
                      the agreement with any applicable local laws. If you
                      disagree with any of these terms, you are prohibited from
                      accessing this site. The materials contained in this
                      Website are protected by copyright and trade mark law.
                    </p>
                  </div>

                  <h2 className="text-xl lg:text-2xl font-semibold mb-3 lg:mb-5">
                    2. Use License
                  </h2>

                  <div className="mb-3 lg:mb-5 max-w-3xl">
                    <p className="text-gray-500 xl:text-lg leading-relaxed text-justify mb-5">
                      Permission is granted to temporarily download one copy of
                      the materials on PT Wikusama Hotel Tbk&apos;s Website for
                      personal, non-commercial transitory viewing only. This is
                      the grant of a license, not a transfer of title, and under
                      this license you may not:
                    </p>

                    <ul className="list-disc px-4 text-gray-500 mb-5">
                      <li>modify or copy the materials;</li>
                      <li>
                        use the materials for any commercial purpose or for any
                        public display;
                      </li>
                      <li>
                        attempt to reverse engineer any software contained on PT
                        Wikusama Hotel Tbk&apos;s Website;
                      </li>
                      <li>
                        remove any copyright or other proprietary notations from
                        the materials; or
                      </li>
                      <li>
                        transferring the materials to another person or
                        &quot;mirror&quot; the materials on any other server.
                      </li>
                    </ul>

                    <p className="text-gray-500 xl:text-lg leading-relaxed text-justify mb-5">
                      This will let PT Wikusama Hotel Tbk to terminate upon
                      violations of any of these restrictions. Upon termination,
                      your viewing right will also be terminated and you should
                      destroy any downloaded materials in your possession
                      whether it is printed or electronic format.
                    </p>
                  </div>

                  <h2 className="text-xl lg:text-2xl font-semibold mb-3 lg:mb-5">
                    3. Disclaimer
                  </h2>

                  <div className="mb-3 lg:mb-5 max-w-3xl">
                    <p className="text-gray-500 xl:text-lg leading-relaxed text-justify mb-5">
                      All the materials on PT Wikusama Hotel Tbk&apos;s Website
                      are provided &quot;as is&quot;. PT Wikusama Hotel Tbk
                      makes no warranties, may it be expressed or implied,
                      therefore negates all other warranties. Furthermore, PT
                      Wikusama Hotel Tbk does not make any representations
                      concerning the accuracy or reliability of the use of the
                      materials on its Website or otherwise relating to such
                      materials or any sites linked to this Website.
                    </p>
                  </div>

                  <h2 className="text-xl lg:text-2xl font-semibold mb-3 lg:mb-5">
                    4. Limitations
                  </h2>

                  <div className="mb-3 lg:mb-5 max-w-3xl">
                    <p className="text-gray-500 xl:text-lg leading-relaxed text-justify mb-5">
                      PT Wikusama Hotel Tbk or its suppliers will not be hold
                      accountable for any damages that will arise with the use
                      or inability to use the materials on PT Wikusama Hotel
                      Tbk&apos;s Website, even if PT Wikusama Hotel Tbk or an
                      authorize representative of this Website has been
                      notified, orally or written, of the possibility of such
                      damage. Some jurisdiction does not allow limitations on
                      implied warranties or limitations of liability for
                      incidental damages, these limitations may not apply to
                      you.
                    </p>
                  </div>

                  <h2 className="text-xl lg:text-2xl font-semibold mb-3 lg:mb-5">
                    5. Revisions and Errata
                  </h2>

                  <div className="mb-3 lg:mb-5 max-w-3xl">
                    <p className="text-gray-500 xl:text-lg leading-relaxed text-justify mb-5">
                      The materials appearing on PT Wikusama Hotel Tbk&apos;s
                      Website may include technical, typographical, or
                      photographic errors. PT Wikusama Hotel Tbk will not
                      promise that any of the materials in this Website are
                      accurate, complete, or current. PT Wikusama Hotel Tbk may
                      change the materials contained on its Website at any time
                      without notice. PT Wikusama Hotel Tbk does not make any
                      commitment to update the materials.
                    </p>
                  </div>

                  <h2 className="text-xl lg:text-2xl font-semibold mb-3 lg:mb-5">
                    6. Links
                  </h2>

                  <div className="mb-3 lg:mb-5 max-w-3xl">
                    <p className="text-gray-500 xl:text-lg leading-relaxed text-justify mb-5">
                      PT Wikusama Hotel Tbk has not reviewed all of the sites
                      linked to its Website and is not responsible for the
                      contents of any such linked site. The presence of any link
                      does not imply endorsement by PT Wikusama Hotel Tbk of the
                      site. The use of any linked website is at the user&apos;s
                      own risk.
                    </p>
                  </div>

                  <h2 className="text-xl lg:text-2xl font-semibold mb-3 lg:mb-5">
                    7. Site Terms of Use Modifications
                  </h2>

                  <div className="mb-3 lg:mb-5 max-w-3xl">
                    <p className="text-gray-500 xl:text-lg leading-relaxed text-justify mb-5">
                      PT Wikusama Hotel Tbk may revise these Terms of Use for
                      its Website at any time without prior notice. By using
                      this Website, you are agreeing to be bound by the current
                      version of these Terms and Conditions of Use.
                    </p>
                  </div>

                  <h2 className="text-xl lg:text-2xl font-semibold mb-3 lg:mb-5">
                    8. Your Privacy
                  </h2>

                  <div className="mb-3 lg:mb-5 max-w-3xl">
                    <p className="text-gray-500 xl:text-lg leading-relaxed text-justify mb-5">
                      Please read our Privacy Policy.{' '}
                      <Link href="/privacy-policy" legacyBehavior>
                        <a className="text-blue-500 hover:text-blue-600">
                          Click here.
                        </a>
                      </Link>
                    </p>
                  </div>

                  <h2 className="text-xl lg:text-2xl font-semibold mb-3 lg:mb-5">
                    9. Governing Law
                  </h2>

                  <div className="mb-3 lg:mb-5 max-w-3xl">
                    <p className="text-gray-500 xl:text-lg leading-relaxed text-justify mb-5">
                      Any claim related to PT Wikusama Hotel Tbk&apos;s Website
                      shall be governed by the laws of id without regards to its
                      conflict of law provisions.
                    </p>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </section>
      </main>
    </>
  );
};

export default ContainerTermsOfService;
